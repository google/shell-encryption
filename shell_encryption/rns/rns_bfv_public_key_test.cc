...
class RnsBfvPublicKeyTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ...
    // BFV encoders currently only support odd plaintext modulus.
    if (rns_params.t % 2 == 0) {
      rns_params.t++;
    }
  ...
  bool IsParameterTooSmallForPublicKeyEncryption(
    ...
    double expected_error = 16.0 * kVariance * (1 << params.log_n);
    double expected_error_bits = std::ceil(std::log2(expected_error));
    double expected_ptxt_bits =
        std::ceil(std::log2(static_cast<double>(params.t)));
    return expected_error_bits + expected_ptxt_bits + 1 < main_modulus_bits;
  }
...
TYPED_TEST_SUITE(RnsBfvPublicKeyTest, testing::ModularIntTypes);

template <typename ModularInt>
class RnsBfvPublicKeyNegativeTest : public RnsBfvPublicKeyTest<ModularInt> {};

TYPED_TEST_SUITE(RnsBfvPublicKeyNegativeTest,
                 testing::ModularIntTypesForNegativeTests);

TYPED_TEST(RnsBfvPublicKeyNegativeTest, CreateFailsIfVarianceIsNotPositive) {
  EXPECT_THAT(RnsBfvPublicKey<TypeParam>::Create(*this->secret_key_,
...
TYPED_TEST(RnsBfvPublicKeyNegativeTest, CreateFailsIfPrngTypeIsInvalid) {
  EXPECT_THAT(RnsBfvPublicKey<TypeParam>::Create(*this->secret_key_, kVariance,
                                                 PRNG_TYPE_INVALID),
              StatusIs(absl::StatusCode::kInvalidArgument,
                       HasSubstr("PrngType not specified correctly")));
}

TYPED_TEST(RnsBfvPublicKeyNegativeTest, EncryptFailsWithNullEncoder) {
  using Encoder = CoefficientEncoder<TypeParam>;

  ASSERT_OK_AND_ASSIGN(RnsBfvPublicKey<TypeParam> public_key,
                       RnsBfvPublicKey<TypeParam>::Create(
...
TYPED_TEST(RnsBfvPublicKeyNegativeTest, EncryptFailsWithNullErrorParams) {
  ASSERT_OK_AND_ASSIGN(RnsBfvPublicKey<TypeParam> public_key,
                       RnsBfvPublicKey<TypeParam>::Create(
                           *this->secret_key_, kVariance, kPrngType));
...
TYPED_TEST(RnsBfvPublicKeyNegativeTest, EncryptFailsWithNullPrng) {
  ASSERT_OK_AND_ASSIGN(RnsBfvPublicKey<TypeParam> public_key,
                       RnsBfvPublicKey<TypeParam>::Create(
                           *this->secret_key_, kVariance, kPrngType));
  EXPECT_THAT(public_key.Encrypt(
...
TYPED_TEST(RnsBfvPublicKeyTest, CreateSucceeds) {
  ASSERT_OK_AND_ASSIGN(RnsBfvPublicKey<TypeParam> public_key,
                       RnsBfvPublicKey<TypeParam>::Create(
                           *this->secret_key_, kVariance, kPrngType));
...
TYPED_TEST(RnsBfvPublicKeyTest, EncryptDecrypts) {
  ...
  ASSERT_OK_AND_ASSIGN(
      ...
      this->secret_key_->DecryptBfv(ciphertext, this->encoder_.get()));
