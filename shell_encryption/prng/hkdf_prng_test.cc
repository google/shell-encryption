...
class HkdfPrngTest : public ::testing::Test {
 protected:
  void SetUp() override {
    ASSERT_OK_AND_ASSIGN(seed_, HkdfPrng::GenerateSeed());
    ASSERT_OK_AND_ASSIGN(prng_, HkdfPrng::Create(seed_));
  }

  std::string seed_;
  std::unique_ptr<HkdfPrng> prng_;
};

TEST_F(HkdfPrngTest, TestRand8BeforeAndAfterResalting) {
...
TEST_F(HkdfPrngTest, TestRand64BeforeAndAfterResalting) {
...
TEST_F(HkdfPrngTest, TestRand64WithResaltingInBetween) {
